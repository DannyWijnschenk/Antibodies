<template>
  ...
  <div class="ps-2 pe-2">  <!-- padding start and padding end of 2 pixels -->
    <div class="container-fluid card ps-2" >  <!-- inner padding of 2 pixels -->
      ...
      <div class="card-body">
        <div class="row">
          <grid-data ref="grid" titleHeading="Abbreviation" table="Abbreviation" v-on:gridClickRow="gridclickrow"></grid-data>
        </div>
      </div>
    </div>
  </div>  <!--card -->
...
export default {
  ...
  methods: {
    ...
    editParameter(id) {
      this.$router.push('/wordabbreviation/'+id);
    },
    addParameter() {
      this.$router.push('/wordabbreviation/new');
    },
